...
class Elastic::Extensions::TestClusterTest < Test::Unit::TestCase
  ...
  context "The Test::Cluster" do
    context "module" do
      should "delegate the methods to the class" do
        ...
        Elastic::Extensions::Test::Cluster.start foo: 'bar'
        Elastic::Extensions::Test::Cluster.stop foo: 'bar'
        Elastic::Extensions::Test::Cluster.running? foo: 'bar'
        Elastic::Extensions::Test::Cluster.wait_for_green foo: 'bar'
      end
    end

    context "class" do
      setup do
        Elastic::Extensions::Test::Cluster::Cluster.any_instance.stubs(:__default_network_host).returns('_local_')

        @subject = Elastic::Extensions::Test::Cluster::Cluster.new(number_of_nodes: 1)
      ...
      should "have a default cluster name" do
        ...
        assert_equal 'elastic-test-foobar', Cluster::Cluster.new.__default_cluster_name
      end
      ...
      context "when stopping a cluster" do
        setup do
          @subject = Elastic::Extensions::Test::Cluster::Cluster.new
        end
      ...
      context "when determining a version" do
        setup do
          @subject = Elastic::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elastic'
        end

        should "return version from lib/elastic.X.Y.Z.jar" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['foo.jar', 'elastic-2.3.0.jar'])
        ...
        should "return version from arguments" do
          cluster = Elastic::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elastic', version: '5.2'
        ...
        should "raise an exception when the version cannot be parsed from .jar" do
          ...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['elastic-100.jar'])
        ...
        should "raise an exception when the version cannot be converted to short version" do
          ...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['elastic-3.2.1.jar'])
