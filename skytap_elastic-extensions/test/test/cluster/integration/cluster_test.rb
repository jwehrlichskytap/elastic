...
class SkytapElastic::Extensions::TestClusterIntegrationTest < Test::Unit::TestCase
  context "The Test::Cluster" do
    ...
    @builds.each do |build|
      should "start and stop #{build.to_s}" do
        ...
        SkytapElastic::Extensions::Test::Cluster.start command: PATH_TO_BUILDS.join(build.join('bin/elastic')).to_s
        SkytapElastic::Extensions::Test::Cluster.stop command: PATH_TO_BUILDS.join(build.join('bin/elastic')).to_s
      end
    end
  end
end
