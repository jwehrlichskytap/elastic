...
class SkytapElastic::Extensions::TestClusterTest < Test::Unit::TestCase
  ...
  context "The Test::Cluster" do
    context "module" do
      should "delegate the methods to the class" do
        ...
        SkytapElastic::Extensions::Test::Cluster.start foo: 'bar'
        SkytapElastic::Extensions::Test::Cluster.stop foo: 'bar'
        SkytapElastic::Extensions::Test::Cluster.running? foo: 'bar'
        SkytapElastic::Extensions::Test::Cluster.wait_for_green foo: 'bar'
      end
    end

    context "class" do
      setup do
        SkytapElastic::Extensions::Test::Cluster::Cluster.any_instance.stubs(:__default_network_host).returns('_local_')

        @subject = SkytapElastic::Extensions::Test::Cluster::Cluster.new(number_of_nodes: 1)
      ...
      should "have a default cluster name" do
        ...
        assert_equal 'skytap_elastic-test-foobar', Cluster::Cluster.new.__default_cluster_name
      end
      ...
      should "remove cluster data" do
        ...
        FileUtils.expects(:rm_rf).with("/tmp/elastic_test")
      ...
      context "when stopping a cluster" do
        setup do
          @subject = SkytapElastic::Extensions::Test::Cluster::Cluster.new
        end
      ...
      context "when determining a version" do
        setup do
          @subject = SkytapElastic::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elastic'
        end

        should "return version from lib/elastic.X.Y.Z.jar" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['foo.jar', 'skytap_elastic-2.3.0.jar'])

          assert_equal '2.0', @subject.__determine_version
        end

        should "return version from `elastic --version`" do
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(false)
          File.expects(:exist?).with('/foo/bar/bin/elastic').returns(true)
        ...
        should "return version from arguments" do
          cluster = SkytapElastic::Extensions::Test::Cluster::Cluster.new command: '/foo/bar/bin/elastic', version: '5.2'
        ...
        should "raise an exception when the version cannot be parsed from .jar" do
          ...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['skytap_elastic-100.jar'])
        ...
        should "raise an exception when the version cannot be converted to short version" do
          # There's no Elastic version 3...
          File.expects(:exist?).with('/foo/bar/bin/../lib/').returns(true)
          Dir.expects(:entries).with('/foo/bar/bin/../lib/').returns(['skytap_elastic-3.2.1.jar'])
        ...
        should "raise an exception when the command cannot be found" do
          ...
          File.expects(:exist?).with('elastic').returns(false)
