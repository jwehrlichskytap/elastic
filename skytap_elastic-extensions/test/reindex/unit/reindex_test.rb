...
class SkytapElastic::Extensions::ReindexTest < Test::Unit::TestCase
  context "The Reindex extension module" do
    ...
    should "require options" do
      assert_raise ArgumentError do
        SkytapElastic::Extensions::Reindex.new
      end
    end

    should "allow to initialize the class" do
      assert_instance_of SkytapElastic::Extensions::Reindex::Reindex,
                         SkytapElastic::Extensions::Reindex.new(DEFAULT_OPTIONS)
    end

    should "add the reindex to the API and client" do
      assert_includes SkytapElastic::API::Actions.public_instance_methods.sort, :reindex
      assert_respond_to SkytapElastic::Client.new, :reindex
    end

    should "pass the client when used in API mode" do
      client = SkytapElastic::Client.new

      SkytapElastic::Extensions::Reindex::Reindex
        .expects(:new)
    ...
    context "when performing the operation" do
      ...
      should "scroll through the index and save batches in bulk" do
        client  = mock()
        subject = SkytapElastic::Extensions::Reindex.new source: { index: 'foo', client: client },
                                                         target: { index: 'bar' }
      ...
      should "transform the documents with a lambda" do
        client  = mock()
        subject = SkytapElastic::Extensions::Reindex.new \
