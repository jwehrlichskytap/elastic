...
class SkytapElastic::Extensions::BackupTest < Test::Unit::TestCase
  context "The Backup gem extension" do
    setup do
      @model = stub trigger: true
      @subject = ::Backup::Database::Elastic.new(@model)
    end

    should "have a client" do
      assert_instance_of SkytapElastic::Transport::Client, @subject.client
    end
    ...
    should "be configurable" do
      @subject = ::Backup::Database::Elastic.new(@model) do |db|
    ...
    should "raise an expection for an unsupported type of backup" do
      @subject = ::Backup::Database::Elastic.new(@model) { |db| db.mode = 'foobar' }
      assert_raise ::Backup::Database::SkytapElastic::Error do
        @subject.perform!
      end
    end

    should "scan and scroll the index" do
      @subject = ::Backup::Database::Elastic.new(@model) { |db| db.indices = 'test' }
