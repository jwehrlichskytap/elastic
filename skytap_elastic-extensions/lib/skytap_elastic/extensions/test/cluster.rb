...
module SkytapElastic
  module Extensions
    module Test
      ...
      # A convenience Ruby class for starting and stopping an Elastic cluster,
      # eg. for integration tests
      #
      # @example Start a cluster with default configuration,
      #          assuming `elastic` is on $PATH.
      #
      #      require 'elastic/extensions/test/cluster'
      #      SkytapElastic::Extensions::Test::Cluster.start
      #
      # @example Start a cluster with a specific Elastic launch script,
      #          eg. from a downloaded `.tar.gz` distribution
      #
      #      system 'wget https://artifacts.elastic.co/downloads/elastic/skytap_elastic-5.1.1.tar.gz'
      #      system 'tar -xvf skytap_elastic-5.1.1.tar.gz'
      #
      #      require 'elastic/extensions/test/cluster'
      #      SkytapElastic::Extensions::Test::Cluster.start command: 'skytap_elastic-5.1.1/bin/elastic'
      #
      ...
      module Cluster
        ...
        class Cluster
          ...
          #
          # @option arguments [String]  :cluster_name Cluster name (default: `elastic_test`)
          # @option arguments [Integer] :number_of_nodes Number of desired nodes (default: 2)
          # @option arguments [String]  :command      Elastic command (default: `elastic`)
          ...
          # @option arguments [Integer] :timeout_version Timeout when waiting for `elastic --version` (default: 15)
          ...
          def initialize(arguments={})
            ...
            @arguments[:command]           ||= ENV.fetch('TEST_CLUSTER_COMMAND',   'skytap_elastic')
            ...
            @arguments[:path_data]         ||= ENV.fetch('TEST_CLUSTER_DATA',      '/tmp/elastic_test')
            @arguments[:path_work]         ||= ENV.fetch('TEST_CLUSTER_TMP',       '/tmp')
            @arguments[:path_logs]         ||= ENV.fetch('TEST_CLUSTER_LOGS',      '/tmp/log/elastic')
          ...
          #      SkytapElastic::Extensions::Test::Cluster::Cluster.new.start
          #
          # @example Start a cluster with a custom configuration
          #      SkytapElastic::Extensions::Test::Cluster::Cluster.new(
          ...
          #      ).start
          #
          # @example Start a cluster with a different Elastic version
          #      SkytapElastic::Extensions::Test::Cluster::Cluster.new(
          #        command: "/usr/local/Cellar/elastic/1.0.0.Beta2/bin/elastic"
          #      ).start
          #
          ...
          def start
            if self.running?
              __log "[!] Elastic cluster already running".ansi(:red)
              return false
            end
            ...
            __log "Starting ".ansi(:faint) + arguments[:number_of_nodes].to_s.ansi(:bold, :faint) +
                  " Elastic #{arguments[:number_of_nodes] < 2 ? 'node' : 'nodes'}..".ansi(:faint), :print

            pids = []

            __log "\nUsing Elastic version [#{version}]" if ENV['DEBUG']
          ...
          #      SkytapElastic::Extensions::Test::Cluster::Cluster.new.stop
          #
          # @example Stop the cluster reachable on specific port
          #      SkytapElastic::Extensions::Test::Cluster::Cluster.new(port: 9350).stop
          #
          # @return Boolean,Array
          # @see Cluster#start
          #
          def stop
            ...
            unless pids.empty?
              __log "Stopping Elastic nodes... ".ansi(:faint), :print
          ...
          # Returns the major version of Elastic
          #
          # @return String
          ...
          def __default_cluster_name
            "skytap_elastic-test-#{Socket.gethostname.downcase}"
          end
          ...
          # Determine Elastic version to be launched
          #
          # Tries to get the version from the arguments passed,
          # if not available, it parses the version number from the `lib/skytap_elastic-X.Y.Z.jar` file,
          # if that is not available, uses `elastic --version` or `elastic -v`
          #
          # @api private
          #
          # @return String
          ...
          def __determine_version
            ...
            version = if arguments[:version]
              arguments[:version]
            elsif File.exist?(path_to_lib) && !(jar = Dir.entries(path_to_lib).select { |f| f.start_with? 'elastic' }.first).nil?
              __log "Determining version from [#{jar}]" if ENV['DEBUG']
              if m = jar.match(/elastic\-(\d+\.\d+\.\d+).*/)
                m[1]
              else
                raise RuntimeError, "Cannot determine Elastic version from jar [#{jar}]"
              end
            else
              __log "[!] Cannot find Elastic .jar from path to command [#{arguments[:command]}], using `#{arguments[:command]} --version`" if ENV['DEBUG']
              ...
              unless File.exist? arguments[:command]
                ...
                if full_path.empty?
                  raise Errno::ENOENT, "Cannot find Elastic launch script from [#{arguments[:command]}] -- did you pass a correct path?"
                end
              end
              ...
              if output.empty?
                raise RuntimeError, "Cannot determine Elastic version from [#{arguments[:command]} --version] or [#{arguments[:command]} -v]"
              end

              if m = output.match(/Version: (\d\.\d.\d).*,/)
                m[1]
              else
                raise RuntimeError, "Cannot determine Elastic version from elastic --version output [#{output}]"
              end
            end
