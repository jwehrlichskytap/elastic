  ...
  module DSL
    module Search
      module Filters
        ...
        #     end
        #
        # @see http://elastic.org/guide/en/elastic/reference/current/query-dsl-has-parent-filter.html
        #
        class HasParent
          ...
          def query(*args, &block)
            @query = block ? @query = SkytapElastic::DSL::Search::Query.new(*args, &block) : args.first
            self
          end
          ...
          def filter(*args, &block)
            @filter = block ? SkytapElastic::DSL::Search::Filter.new(*args, &block) : args.first
            self
          end
