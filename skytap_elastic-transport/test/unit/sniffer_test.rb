...
class SkytapElastic::Transport::Transport::SnifferTest < Test::Unit::TestCase
  ...
  def __nodes_info(json)
    SkytapElastic::Transport::Transport::Response.new 200, MultiJson.load(json)
  end

  context "Sniffer" do
    setup do
      @transport = DummyTransport.new
      @sniffer   = SkytapElastic::Transport::Transport::Sniffer.new @transport
    end
    ...
    should "return an array of hosts as hashes with Elastic 1.x syntax" do
      @transport.expects(:perform_request).returns __nodes_info <<-JSON
        {
          "ok" : true,
          "cluster_name" : "elastic_test",
          "nodes" : {
            "N1" : {
              "name" : "Node 1",
    ...
    should "return an array of hosts as hashes with Elastic 2.0 syntax" do
      @transport.expects(:perform_request).returns __nodes_info <<-JSON
        {
          "ok" : true,
          "cluster_name" : "elastic_test",
    ...
    should "raise error on timeout" do
      @transport.expects(:perform_request).raises(SkytapElastic::Transport::Transport::SnifferTimeoutError)

      # TODO: Try to inject sleep into `perform_request` or make this test less ridiculous anyhow...
      assert_raise SkytapElastic::Transport::Transport::SnifferTimeoutError do
        @sniffer.hosts
      end
    end
