...
# Launch test cluster
#
if ENV['SERVER'] and not Elastic::Extensions::Test::Cluster.running?
  ...
  Elastic::Extensions::Test::Cluster.start(nodes: 1, es_params: es_params )
end

# Register `at_exit` handler for server shutdown.
# MUST be called before requiring `test/unit`.
#
at_exit { Elastic::Extensions::Test::Cluster.stop if ENV['SERVER'] and Elastic::Extensions::Test::Cluster.running? }
...
tracer.progname = 'elastic.tracer'
...
# Store Elastic version
#
es_version_info = $client.info['version']
$es_version = es_version_info['number']

puts '-'*80,
     "Elastic #{$es_version.ansi(:bold)} [#{es_version_info['build_hash'].to_s[0...7]}]".center(80),
     '-'*80
...
suites.each do |suite|
  name = Elastic::YamlTestSuite::Utils.titleize(suite.basename)

  Elastic::YamlTestSuite::Runner.in_context name do
