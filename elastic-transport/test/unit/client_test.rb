...
class Elastic::Transport::ClientTest < Test::Unit::TestCase
  ...
  context "Client" do
    setup do
      Elastic::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.stubs(:__build_connections)
      @client = Elastic::Transport::Client.new
    end

    should "be aliased as Elastic::Client" do
      assert_nothing_raised do
        assert_instance_of(Elastic::Transport::Client, Elastic::Client.new)
      end
    end

    should "have default transport" do
      assert_instance_of Elastic::Transport::Client::DEFAULT_TRANSPORT_CLASS, @client.transport
    end

    should "instantiate custom transport class" do
      client = Elastic::Transport::Client.new :transport_class => DummyTransport
      assert_instance_of DummyTransport, client.transport
    end

    should "take custom transport instance" do
      client = Elastic::Transport::Client.new :transport => DummyTransport.new
      assert_instance_of DummyTransport, client.transport
    end
    ...
    should "send GET request as POST with the send_get_body_as option" do
      ...
      client = Elastic::Transport::Client.new :transport => transport, :send_get_body_as => 'POST'
    ...
    context "when passed hosts" do
      should "have localhost by default" do
        c = Elastic::Transport::Client.new
        assert_equal 'localhost', c.transport.hosts.first[:host]
      end

      should "take :hosts, :host, :url or :urls" do
        c1 = Elastic::Transport::Client.new :hosts => ['foobar']
        c2 = Elastic::Transport::Client.new :host  => 'foobar'
        c3 = Elastic::Transport::Client.new :url   => 'foobar'
        c4 = Elastic::Transport::Client.new :urls  => 'foo,bar'
    ...
    context "detecting adapter for Faraday" do
      setup do
        Elastic::Transport::Client::DEFAULT_TRANSPORT_CLASS.any_instance.unstub(:__build_connections)
      ...
      should "use the adapter from arguments" do
        c = Elastic::Transport::Client.new :adapter => :typhoeus
        handlers = c.transport.connections.all.first.connection.builder.handlers
    ...
    context "configuring Faraday" do
      ...
      should "apply faraday adapter" do
        c = Elastic::Transport::Client.new do |faraday|
    ...
    context "when passed options" do
      ...
      should "transfer selected host parts from URL into the 'http' options" do
        c = Elastic::Transport::Client.new :url => 'https://U:P@node1:8080'
