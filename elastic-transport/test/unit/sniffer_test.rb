...
class Elastic::Transport::Transport::SnifferTest < Test::Unit::TestCase
  ...
  def __nodes_info(json)
    Elastic::Transport::Transport::Response.new 200, MultiJson.load(json)
  end

  context "Sniffer" do
    setup do
      @transport = DummyTransport.new
      @sniffer   = Elastic::Transport::Transport::Sniffer.new @transport
    end
    ...
    should "raise error on timeout" do
      @transport.expects(:perform_request).raises(Elastic::Transport::Transport::SnifferTimeoutError)

      # TODO: Try to inject sleep into `perform_request` or make this test less ridiculous anyhow...
      assert_raise Elastic::Transport::Transport::SnifferTimeoutError do
        @sniffer.hosts
      end
    end
