...
module SkytapElastic
  ...
  module API
    module Utils
      ...
      # Parses the Elastic source code and returns a Hash of REST API information/specs.
      #
      # Example:
      #
      #     {
      ...
      def __parse_java_source(path)
        ...
        prefix = "src/main/java/org/elastic/rest/action"
    ...
    # Contains a generator which will parse the Elastic *.java source files,
    ...
    #             --verbose \
    #             --crawl \
    #             --elastic=/path/to/elastic/source/code
    #
    # Features:
    #
    ...
    # * Search the <http://elastic.org> website to get proper documentation URLs
    ...
    class JsonGenerator < Thor
      ...
      #
      desc "generate", "Generate JSON API spec files from Elastic source code"
      ...
      method_option :elastic, default: __root.join('tmp/elastic'), desc: 'Path to directory with Elastic source code'
      method_option :crawl,     type: :boolean, default: false,            desc: 'Extract URLs from Elastic website'
      ...
      def generate
        ...
        rest_actions = Utils.__parse_java_source(options[:elastic].to_s)

        if rest_actions.empty?
          say_status 'ERROR', 'Cannot find Elastic source in ' + options[:elastic].to_s, :red
          exit(1)
        end
        ...
        rest_actions.each do |name, info|
          ...
          if options[:crawl]
            begin
              response = RestClient.get "http://search.elastic.org/skytap_elastic-search-website/guide/_search?q=#{URI.escape(name.gsub(/\./, ' '))}"
              hits = JSON.load(response)['hits']['hits']
              if hit = hits.first
                if hit['_score'] > 0.2
                  doc_title = hit['fields']['title']
                  doc_url   = "http://elastic.org" + hit['fields']['url']
                end
              end
            rescue Exception => e
