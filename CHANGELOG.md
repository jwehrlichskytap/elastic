...
* Added deprecation notices to API methods and parameters not supported on Elastic 2.x
...
* Updated the `elastic:build` Rake task
...
* Updated the README example to work with Elastic 2.x
...
* Added information about configuring the client for the Amazon Elastic Service
...
* Clarified version support of Elastic
* Improved the `elastic:build` Rake task to work with branch names
...
* Define the path to core REST tests dynamically based on Elastic version
...
* Added, that connection reloading supports Elastic 2.0 output
...
* Added the integration with Elastic Watcher plugin
...
* Updated the Curb integration test to not fail on older Elastic versions
...
* Fixed incorrect dependency specification in the "elastic" wrapper gem
...
* Improved the `elastic:build` Rake task
